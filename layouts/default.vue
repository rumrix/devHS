<template>
  <div>
    <b-navbar toggleable="lg" type="dark" variant="dark">
      <b-navbar-brand href="/">
        <img src="~/assets/logo.svg" style="width: 50px; height: 50px" />
        Home
      </b-navbar-brand>
      <b-navbar-toggle target="nav-collapse"></b-navbar-toggle>

      <b-collapse id="nav-collapse" is-nav>
        <b-navbar-nav>
          <b-nav-item href="/axios">Select</b-nav-item>
          <b-nav-item href="/router-middleware">Insert</b-nav-item>
          <b-nav-item href="/address">Address</b-nav-item>
          <b-nav-item href="/bootstrap-vue">B Vue</b-nav-item>
          <b-nav-item href="/file-upload-download">File</b-nav-item>
        </b-navbar-nav>

        <!-- Right aligned nav items -->
        <b-navbar-nav class="ml-auto">
          <b-nav-item>
            {{ $store.state.signin.id }}
          </b-nav-item>
          <b-nav-item>
            {{ $store.state.signin.password }}
          </b-nav-item>
          <b-button size="sm" class="my-2 my-sm-0" @click="onSignout"
            >Reset</b-button
          >
          &nbsp;
          <b-nav-form>
            <b-form-input
              size="sm"
              class="mr-sm-2"
              placeholder="Search"
            ></b-form-input>
            <b-button size="sm" class="my-2 my-sm-0" type="submit"
              >Search</b-button
            >
          </b-nav-form>
        </b-navbar-nav>
      </b-collapse>
    </b-navbar>
    <main>
      <Nuxt />
    </main>
    <b-navbar class="footer" toggleable="lg" type="dark" variant="dark">
      <b-navbar-brand>KHS TEST</b-navbar-brand>
    </b-navbar>
  </div>
</template>
<script>
export default {
  data() {
    return {}
  },
  methods: {
    onSignout() {
      this.$store.commit('signin/idstored', null)
      this.$store.commit('signin/pwstored', null)
    },
  },
}
</script>

<style scoped>
.footer {
  bottom: 0;
}
</style>
